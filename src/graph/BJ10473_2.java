...
public class BJ10473_2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while(!pq.isEmpty()) {
            ...
            for (int i = 0; i < N + 2; i++) {
                ...
                double costToNext = distance[nowNum] + getTime(nowNum, i);
                if (costToNext < distance[i]) {
                    distance[i] = costToNext;
    ...
    public static double getTime(int nowNum, int nextNum) {
        double d = getDis(nodes[nowNum].x, nodes[nowNum].y,
                nodes[nextNum].x, nodes[nextNum].y);

        double runTime = d / 5.0;

        if (nowNum >= 1 && nowNum <= N) {
            double cannonTime = 2.0 + Math.max(0.0, d - 50.0) / 5.0;
